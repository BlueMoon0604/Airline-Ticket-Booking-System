...
<<<<<<< HEAD
    ...
    private Role resolveRegistrationRole(String email, Role requestedRole) {
        ...
        if (requestedRole == Role.ADMIN) {
            throw new BadRequestException("Only the designated master admin account can hold the ADMIN role");
        }

        return requestedRole == null ? Role.PASSENGER : requestedRole;
    }

    private User normalizeAdminRole(User user) {
        if (user == null) {
            return null;
        }
        ...
        if (isOnlyAdminEmail(normalizedEmail) && currentRole != Role.ADMIN) {
    ...
    private boolean isOnlyAdminEmail(String email) {
        return ONLY_ADMIN_EMAIL.equals(normalizeEmail(email));
    }
